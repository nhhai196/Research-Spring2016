...
\begin{proof}
Let $S$ be the set of all the probability distributions $X$ with $H_{\infty}(X) \geq k$, then S is a compact convex polytope in $\mathbb{R}^N$. The set of all $2^k$-flat distributions is the set of vertices (extreme points) of $S$. In a compact convex set, every point can be written as a convex combination of its vertices. Hence, every distribution $X$ with  $H_{\infty}(X) \geq k$ can be written as a convex combination of $k$-flat distributions. 
