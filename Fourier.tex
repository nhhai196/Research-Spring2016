...
$$SD \{ (XG_0, XG \oplus M, G_0, G), (U, XG \oplus M, G_0, G) \} \leq  \cdots $$
\end{theorem}

\begin{proof}
For convenience, let $A = (XG_0, XG \oplus M | G_0 , G)$ and 
...
\begin{claim}
For any $S \subseteq [n+1]$, $\widehat{A}(S) = \widehat{B}(S)$ if $S_1 = 0$, and 
$\widehat{B}(S) = 0$ if $S_1 = 1$.
\end{claim}
