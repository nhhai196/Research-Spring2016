...
The entire treatment for Fourier analysis can be generalized to functions from any finite abelian group  $\mathbb{G}$ to the set of complex numbers $\mathbb{C}$.
...
\end{definition}
The $L^P$ norm of $f$ is defined as $\| f \|_{L^P} = ({\mathbb{E}_x} |f(x)|^p)^{1/p} $
\section{Characters}
\begin{definition} 
A character of $\mathbb{G}$ is a homomorphism $\chi : \mathbb{G} \rightarrow \mathbb{C}$.
\end{definition}
$\chi$ is a trivial (principal) character if $\chi = 1$
